...
------ Câu 2B: Hằng tháng có bao nhiêu khách hàng sử dụng dịch vụ 'trial'
select format(start_date, 'MM-yyyy') as date_month, count(distinct customer_id) as total_customer_use_trial
from subscriptions s
join plans p on s.plan_id = p.plan_id
where plan_name = 'trial'
group by format(start_date, 'MM-yyyy')

------ Câu 3B: Có bao nhiêu khách hàng sử dụng cho mỗi dịch vụ tính từ sau năm 2020
